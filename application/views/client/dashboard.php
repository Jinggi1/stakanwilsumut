...
<div class="wrapper">
  ...
  <aside class="main-sidebar sidebar-dark-primary elevation-4">
    ...
    <div class="sidebar">
      ...
      <div class="user-panel mt-3 pb-3 mb-3 d-flex">
        <div class="info">
          <a href="#" class="d-block"><?=@$_SESSION['nama_klien'];?></a>
        </div>
      </div>
  ...
  <div class="content-wrapper">
    ...
    <?php } else if($this->session->flashdata('create')){  ?>
          ...
          <div class="col-12">
            <div class="card">
              <div class="card-header">
                <h3 class="card-title"><button type="button" class="btn btn-block btn-primary btn-lg" data-toggle="modal" data-target="#modal-create-new-user">Create New Ticket</button></h3>
              </div>
              <!-- /.card-header -->
              <div class="card-body">
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  <?php 
                  $no = 1;
                  foreach($klienticket as $k){ ?>
                  <tr>
                    <td><?php echo $k->tic_no ?></td>
                    <td><?php echo $k->start_date ?></td>
                    <td><?php echo $k->category_name ?></td>
                    <td><?php echo $k->div_name ?></td>
                    <td>
                      <?php 
                      if ($k->status == 1) {
                      echo 'Open';
					...
					<td><?php echo $k->from_klien ?></td>
                    <td>
                      <?php 
                      if ($k->status == 3 AND $k->from_klien == '-') {
                      echo '<button type="button" class="btn btn-block btn-success btn-sm" data-toggle="modal" data-target="#modal-feedback-'.$k->id_tic.'">Feedback</button>';
                      }else{
                        echo '-';
                      }; ?>
                    </td>
                    ...
                    <!-- modal feedback -->
                  <div class="modal fade" id="modal-feedback-<?php echo $k->id_tic ?>">
                      <div class="modal-dialog modal-lg">
                        <div class="modal-content">
                          <div class="modal-header">
                            <h4 class="modal-title">Akan memberi feedback tiket <?php echo $k->tic_no ?>?</h4>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                              <span aria-hidden="true">&times;</span>
                            </button>
                          </div>
                          <div class="modal-body">
                            <?php echo form_open('client/dashboard/feedback'); ?>
                              <div class="card-body">
                                <input type="hidden" id="id_tic" name="id_tic" value="<?php echo $k->id_tic ?>">
                                <div class="form-group">
                                  <label for="exampleInputPassword">Feedback</label>
                                  <input type="text" name="remark_klien" required class="form-control" id="exampleInputName" placeholder="Enter Your Feedback">
                                </div>
                              </div>
      ...
      <div class="modal fade" id="modal-create-new-user">
        <div class="modal-dialog modal-lg">
          <div class="modal-content">
            ...
            <div class="modal-body">
              <?php echo form_open('client/dashboard/create'); ?>
                <div class="card-body">
					
					<input type="hidden" name="id_klien" value="<?=@$_SESSION['id_klien'];?>" id="exampleInputName">
					<input type="hidden" name="id_dept" value="<?=@$_SESSION['dept_klien'];?>" id="exampleInputName">
					<input type="hidden" name="id_loc" value="<?=@$_SESSION['location_klien'];?>" id="exampleInputName">
					<input type="hidden" name="nama_klien" value="<?=@$_SESSION['nama_klien'];?>" id="exampleInputName">
					<input type="hidden" name="telp_klien" value="<?=@$_SESSION['telp_klien'];?>" id="exampleInputName">
					<input type="hidden" name="email_klien" value="<?=@$_SESSION['email_klien'];?>" id="exampleInputName">
					
					<div class="form-group">
                    <label>Seksi</label>
                    <select class="form-control select2" id="division" name="id_div" style="width: 100%;" required>
                      <option selected="selected" value="">-- Pilih Seksi --</option>
                    </select>
                  </div>
				  <div class="form-group">
                    <label>Kategori Kendala</label>
                    <select class="form-control select2" id="category" name="id_cat" style="width: 100%;" required>
                      <option selected="selected" value="">-- Pilih Kategori --</option>
                    </select>
                  </div>
                  <div class="form-group">
                    <label for="exampleInputEmail1">Pesan</label>
                    <textarea class="form-control" rows="6" name="message" id="message" placeholder="Enter your Problem" required></textarea>
                  </div>
                </div>
...
<!-- jQuery -->
<script src="<?php echo base_url(); ?>asset/html5/assets/js/jquery.min.js"></script>
<!-- jQuery UI 1.11.4 -->
...
<script type="text/javascript">
				...
				$(document).ready(function() {
					$("#division").select2({
						ajax: {
							url: '<?= base_url() ?>client/dashboard/getdiv',
							type: "post",
							dataType: 'json',
							delay: 200,
				...
				$("#division").change(function() {
					...
					$("#category").select2({
						ajax: {
							url: '<?= base_url() ?>client/dashboard/getcat/' + id_div,
							type: "post",
							dataType: 'json',
							delay: 200,
							data: function(params) {
